...
var DEBUGURLS = {
  ...
  CLOSE_ORDER: 'https://api.mch.weixin.qq.com/sandboxnew/sandboxnew/pay/closeorder',
  REDPACK_SEND: 'https://api.mch.weixin.qq.com/sandboxnew/mmpaymkttransfers/sendredpack',
  REDPACK_QUERY: 'https://api.mch.weixin.qq.com/sandboxnew/mmpaymkttransfers/gethbinfo',
...
Payment.prototype.sendRedPacket = function(order, callback) {
  ...
  var redpacketSend = this.debug ? DEBUGURLS.REDPACK_SEND : URLS.REDPACK_SEND;
  this._signedQuery(redpacketSend, order, {
    https: true,
...
Payment.prototype.redPacketQuery = function(order, callback) {
  ...
  var redpacketQuery = this.debug ? DEBUGURLS.REDPACK_QUERY : URLS.REDPACK_QUERY;
  this._signedQuery(redpacketQuery, order, {
    https: true,
    required: requiredData
  }, callback);
};
...
Payment.prototype.transfers = function(order, callback) {
  ...
  var transfersURL = this.debug ? DEBUGURLS.TRANSFERS : URLS.TRANSFERS;
  this._signedQuery(transfersURL, order, {
    https: true,
    required: requiredData
  }, callback);
};
...
Payment.prototype._signedQuery = function(url, params, options, callback) {
  ...
  var thisURL = self.debug ? DEBUGURLS : URLS;
  if (url == thisURL.REDPACK_SEND) {
    params = this._extendWithDefault(params, [
      'mch_id',
      'nonce_str'
    ]);
  } else if (url == thisURL.TRANSFERS) {
    params = this._extendWithDefault(params, [
      'nonce_str'
    ]);
  } else {
...
Payment.prototype.unifiedOrder = function(params, callback) {
  ...
  var thisURLS = this.debug ? DEBUGURLS : URLS;
  this._signedQuery(thisURLS.UNIFIED_ORDER, params, {
    required: requiredData
  }, callback);
};
...
Payment.prototype.orderQuery = function(params, callback) {
  var thisURLS = this.debug ? DEBUGURLS : URLS;
  this._signedQuery(thisURLS.ORDER_QUERY, params, {
...
Payment.prototype.refund = function(params, callback) {
  ...
  var thisURLS = this.debug ? DEBUGURLS : URLS;
  this._signedQuery(thisURLS.REFUND, params, {
    https: true,
...
Payment.prototype.refundQuery = function(params, callback) {
  var thisURLS = this.debug ? DEBUGURLS : URLS;
  this._signedQuery(thisURLS.REFUND_QUERY, params, {
...
Payment.prototype.downloadBill = function(params, callback) {
  var self = this;
  var thisURLS = this.debug ? DEBUGURLS : URLS;
  this._signedQuery(thisURLS.DOWNLOAD_BILL, params, {
...
Payment.prototype.shortUrl = function(params, callback) {
  var thisURLS = this.debug ? DEBUGURLS : URLS;
  this._signedQuery(thisURLS.SHORT_URL, params, {
    required: ['long_url']
  }, callback);
};
...
Payment.prototype.closeOrder = function(params, callback) {
  var thisURLS = this.debug ? DEBUGURLS : URLS;
  this._signedQuery(thisURLS.CLOSE_ORDER, params, {
